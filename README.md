...
- Residents can use `public-search`, `public-https`, `public-text`, `public-json`, `public-csv`, and `wikipedia-search`; all are read-only, public HTTPS tools with bounded responses and no credentials. Search stays compact; text and structured research accept up to 128 KB, while public text remains a 2,400-character sanitized excerpt and structured tools publish only shape/metadata. External text is explicitly untrusted.
