...
- Residents can use `public-search`, `public-https`, `public-text`, `public-json`, `public-csv`, and `wikipedia-search`; all are read-only, public HTTPS tools with bounded responses and no credentials. Search links and sanitized text excerpts can guide follow-up; external text is explicitly untrusted. Structured data tools publish only shape/metadata, never raw source payloads.
