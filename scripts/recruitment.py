...
def propose(args):
    ...
    data["proposals"].append({"id": f"recruit-{number:04d}", "name": name, "role": role,
        "card": args.card[:300], "reason": reason, "status": "quarantined", "proposed_at": stamp()})
...
p = commands.add_parser("propose"); p.add_argument("--name", required=True); p.add_argument("--role", required=True); p.add_argument("--card", required=True); p.add_argument("--reason", required=True); p.set_defaults(func=propose)
p = commands.add_parser("list"); p.set_defaults(func=listing)
p = commands.add_parser("review"); p.add_argument("--id", required=True); p.add_argument("--decision", choices=("accepted", "declined"), required=True); p.set_defaults(func=review)
