...
def ask(url, resident, context, retry_reason="", line=None):
    ...
    if line and line.get("root"):
        ...
        established = (("The world has already established about this subject: " + " | ".join(str(item)[:160] for item in known[:5]) +
                        ". Build on those: ask for the next fact that would extend them, or for a source likely to give a different figure. ")
                       if known else "")
