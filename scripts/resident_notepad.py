...
parser.add_argument("--agent", required=True, help="active resident ID from the local registry")
...
try:
    ...
    registry = {"agents": []}
eligible = {item.get("id") for item in registry.get("agents", [])
            if item.get("status") not in {"fired", "retired"}}
if args.agent not in eligible:
    raise SystemExit("note rejected: resident is not active in the local registry")
